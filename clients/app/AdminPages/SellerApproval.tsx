...
const SellerApproval = () => {
  ...
  const handleUpdate = async (id: any) => {
    try {
      await axios.put(`${API_URL}/api/user-update/${id}`, {
        isSeller: isNewSeller,
      });
      fetchUsers();
      setIsNewSeller("");
      setIsUpdate(null);
          ...
          <View key={index}>
            {users.isSeller === "false" && (
              <View className="mx-5">
                ...
                <View className="">
                  <Pressable
                    className="bg-blue-500 p-2 rounded my-2"
                    onPress={() => setIsUpdate(users._id)}
                  >
                ...
                {isUpdate == users._id && (
                  <View className="mx-2 mt-5 border pt-5 ">
                    <View className="grid grid-cols-2">
                      ...
                      <Pressable
                        className="bg-green-700 p-2 w-32"
                        onPress={() => setIsNewSeller("true")}
                      >
                        <Text className="text-white text-center">Approve</Text>
                      </Pressable>

                      <Pressable
                        className="bg-red-700 p-2 w-32"
                        onPress={() => setIsNewSeller("null")}
                      >
                    ...
                    <View className="grid grid-cols-2">
                      ...
                      <Pressable
                        ...
                        onPress={() => handleUpdate(users._id)}
                      >
            ...
            {users.isSeller === "true" && (
              <View>
                ...
                <Text>{users.fullname}</Text>
                <Text>
                  {users.isSeller === "true" && <Text>Verified seller</Text>}
                </Text>
                <Pressable
                  className="bg-blue-500 p-2 rounded my-2"
                  onPress={() => setIsUpdate(users._id)}
