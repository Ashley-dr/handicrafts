...
const handleSignup = async () => {
        ...
        try {
            ...
            if (error.response && error.response.data && error.response.data.error === "Email Already Exist") {
                Alert.alert("Sign up failed", "Account already exists. Please use another email.");
            }
             else {
  ...
  <View style={tw`bg-[#FFFCEF] p-5 rounded-lg w-[90%]`}>
     <Text style={tw`text-[28px] font-bold text-center m-[6px]`}>
      ...
      <TextInput
      value={email} 
       style={tw`bg-[#efdbbb4f] text-[#AD9C8E] text-[12px] rounded-t-lg mb-4 w-full`} label="Email"
       autoCapitalize='none' 
...
<View className="flex-row justify-between mb-4">
      ...
      <Button onPress={handleSignup}  style={tw`bg-[#DFC4A4] rounded-md mb-3`}
          labelStyle={tw`text-[#4f3a3a] text-[13px]`}>Sign up</Button>
